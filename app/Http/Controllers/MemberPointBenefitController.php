...
class MemberPointBenefitController extends Controller
{

public function calculateMemberPointBenefit(){
    try {
    ...
    foreach ($results as $result) {
        ...
        if($result->informationReceiptProduct->informationReceipt->customer && $result->informationReceiptProduct->informationReceipt->customer->member){//if customer is member(have row in Member table)
            ...
            if($member_status == 2){//check member status
                ...
                echo 'M E M B E R P R I C E : '.$memberPrice;
                echo '<br>h s P r o d u c t P r i c e : '.$hsProductPrice;
                echo '<br>P O I N T : '.$point;
                if($memberPrice != null || $memberPrice != 0){
                    echo '<br>TRUE 1';
...
public function calculatePointBenefitHSDoor(){
